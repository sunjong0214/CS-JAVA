...
위의 소스 코드 같이 new 연산으로 선언했을 때 해당 선언한 객체에서는 기본 생성자가 생성된다. 

```java
class BoardMenu {
    void BoardMenu() {
        
    } //기본 생성자 - 선언 시 다른 생성자가 없다면 자동으로 생성 됨
}
```

하지만 우리가 만약 다양한 게시판을 만들고 거기에 제목 같은 필수적으로 들어가는 인스턴스 변수를 강제적으로 지정하고 싶을 때 생성자가 필요하다.
...
class BoardMenu {
    ...
    //기본 생성자 대신 타이틀 변수를 받는 생성자를 만들어 타이틀이라는 인스턴스 변수가 필요하더록 지정
}
class Menu {
